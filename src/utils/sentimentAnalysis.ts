...
export const analyzeSentiment = (text: string): { sentiment: 'positive' | 'negative' | 'neutral', explanation: string } => {
  ...
  if (positiveCount > negativeCount) {
    return { 
      sentiment: 'positive',
      explanation: positiveWords.length > 0
        ? `This text has a positive tone due to words like: ${positiveWords.slice(0, 3).join(', ')}.`
        : "This text has a positive tone due to enthusiastic words." 
    };
  } else if (negativeCount > positiveCount) {
    return { 
      sentiment: 'negative',
      explanation: negativeWords.length > 0
        ? `This text has a negative tone due to words like: ${negativeWords.slice(0, 3).join(', ')}.`
